...
declare
  ...
  inputoffset integer := greatest(
    coalesce(nullif(payload->>'inputoffset', '')::integer, 0),
    0
  );
begin
  ...
  if inputlimit is not null then
    inputlimit := greatest(1, least(inputlimit, 101));
  end if;
  ...
  order by
    ...
    case
      when e.start_date < current_date then current_date
      else e.start_date
    end,
    e.start_time::time nulls last,
    coalesce(e.end_date, e.start_date) nulls last,
    e.country,
    e.city,
    e.location,
    e.name,
...
revoke all on function public.get_filtered_recommended_events(json)
  from public, anon;
grant execute on function public.get_filtered_recommended_events(json)
  to authenticated;
